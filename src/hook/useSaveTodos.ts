  ...
  useEffect(() => {
    try {
      ...
      if (todoData) {
        ...
        const decryptData: Todo[] = JSON.parse(bytes.toString(enc.Utf8));

        dispatch({ type: "ADD", payload: decryptData });
      }
    } catch {
      alert("Não foi possível obter as tarefas!");
    }
